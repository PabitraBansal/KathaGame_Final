...
public class puzzle_supergirls extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_puzzle_supergirls);
        init();
        scramble();
        setDimensions();
        mContext = this;
    ...
    private static void display(Context context) {
        ...
        for (int i=0;i<titleList.length;i++){
            ...
            if(titleList[i].equals("0")){
                button.setBackgroundResource(R.drawable.supergirl_grid_11);
            }
            else if(titleList[i].equals("1")){
                button.setBackgroundResource(R.drawable.supergirl_grid_12);
            }
            else if(titleList[i].equals("2")){
                button.setBackgroundResource(R.drawable.supergirl_grid_13);
            }
            else if(titleList[i].equals("3")){
                button.setBackgroundResource(R.drawable.supergirl_grid_21);
            }
            else if(titleList[i].equals("4")){
                button.setBackgroundResource(R.drawable.supergirl_grid_22);
            }
            else if(titleList[i].equals("5")){
                button.setBackgroundResource(R.drawable.supergirl_grid_23);
            }
            else if(titleList[i].equals("6")){
                button.setBackgroundResource(R.drawable.supergirl_grid_31);
            }
            else if(titleList[i].equals("7")){
                button.setBackgroundResource(R.drawable.supergirl_grid_32);
            }
            else if(titleList[i].equals("8")){
                button.setBackgroundResource(R.drawable.supergirl_grid_33);
            }
    ...
    private void init(){
        mGridView = (GestureDetectGridView2)findViewById(R.id.grid_supergirls);
    ...
    private static void swap(Context context, int position, int swap) {
        ...
        display(context);
        if (isSolved() == true) {
            Intent intent = new Intent(mContext, endscreen_supergirls.class);
